#!/usr/bin/env python3
"""Refresh the example schemas under tests/fixtures/ from the plan (dev only).
...
The plan repo+ref come from omnibenchmark.yaml's `plan` entry; override with
--repo/--ref. If the plan publishes no schema/ yet, this is a no-op.
"""
...
FIXTURES = ROOT / "tests" / "fixtures" / "module" / "src" / "common" / "schema"
    ...
    try:
        ...
        for f in files:
            ...
            if a.check:
                ...
                shutil.copy2(f, dest)
                print(f"refreshed tests/fixtures/module/src/common/schema/{f.name}")
